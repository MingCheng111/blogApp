...
const editInfo = async (req, res) => {
  ...
  jwt.verify(req.token, SECRET, async (err, auth) => {
    ...
    User.findOneAndUpdate(
      { _id: user._id },
      { $set: { additionalData: {...user.additionalData, ...req.body} } }
    ).then((response) => {
        ...
        { _id: user._id },
        {
          $set: {
            additionalData: {
              ...user.additionalData,
              profilePic: urls[0].url,
              coverPhoto: (user.additionalData && user.additionalData.coverPhoto) ? user.additionalData.coverPhoto : "",
            },
          },
        }
      ).then((response) => {
      ...
      User.findOneAndUpdate(
        { _id: user._id },
        {
          $set: {
            additionalData: {
              ...user.additionalData,
              profilePic: (user.additionalData && user.additionalData.profilePic) ? user.additionalData.profilePic : "",
